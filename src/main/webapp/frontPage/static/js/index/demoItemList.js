...
var adoptDog = function(dogId) {
	...
	swal({
		...
		if (isConfirm) {
			...
			$.ajax({
		    	url: hdnContextPath + "/user/adoptDog.action",
		    	type: "POST",
		    	async: false,
		    	data: {"dogId": dogId},
		    	success: function(data) {
		            if(data.status == 200 && data.msg == "OK") {
		            	App.unblockUI();
		            	SweetAlert.success("收养请求发送成功！请等待后台管理员审核");
		            	// 刷新jqgrid
		            	reloadItemGrid();
		            } else {
		            	App.unblockUI();
            ...
            colModel: [  
                ...
                { width: 30, sortable: false,
                	formatter:function(cellvalue, options, rowObject) {
    			    	...
    			    	var btnModify = '<button class="btn btn-outline blue-chambray" id="" onclick=adoptDog("' + dogId + '") style="padding: 1px 3px 1px 3px;">收养</button>';
